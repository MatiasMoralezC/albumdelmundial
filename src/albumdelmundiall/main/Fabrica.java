...
public class Fabrica {
	...
	// GENERAR ALBUMS
	Album crearAlbumWeb() {
		return new AlbumWeb(paisesParticipantes, lugaresPorPais);

	}

	Album crearAlbumExtendido() {
		return new AlbumExtendido(paisesParticipantes, lugaresPorPais);

	}

	Album crearAlbumTradicional() {
		return new AlbumTradicional(paisesParticipantes, lugaresPorPais);
	...
	List<FiguritaTradicional> generarSobre(int cantFigus) {
		List<FiguritaTradicional> sobre = new ArrayList<FiguritaTradicional>();

		for (int i = 0; i < cantFigus; i++) {
			int numJugador = random.nextInt(12); 
			String nombrePais = paisesParticipantes[random.nextInt(paisesParticipantes.length)];
			FiguritaTradicional figurita = new FiguritaTradicional("Tradicional", numJugador, nombrePais, calcularValorBase(nombrePais, numJugador));
			sobre.add(figurita);
		}
	...
	List<FiguritaTradicional> generarSobreTop10(int cantFigus) {
		List<FiguritaTradicional> sobreTop10 = new ArrayList<FiguritaTradicional>();
		...
		for (int i = 0; i < cantFigus; i++) {
			...
			FiguritaTop10 figurita = new FiguritaTop10(nombrePais, numJugador, calcularValorBase(nombrePais, numJugador), paisSede, balon);
	...
	public int valorBase(FiguritaTradicional fig) {
		return calcularValorBase(fig.getNombrePais(), fig.getNumJugador());
	}
	
	public Map<String, Integer> darRanking() {
